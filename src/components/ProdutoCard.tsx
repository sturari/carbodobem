...
export function ProdutoCard({ p }: { p: ProdutoCardData }) {
  ...
  const semEstoque = typeof p.estoque === "number" && p.estoque <= 0;
  const estoqueBaixo =
    typeof p.estoque === "number" && p.estoque > 0 && p.estoque <= 3;
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
        {p.imagem_url ? (
          <img
            ...
            alt={p.nome}
            loading="lazy"
            className={`h-full w-full object-cover transition duration-500 group-hover:scale-105 ${semEstoque ? "opacity-60 grayscale" : ""}`}
          />
        ) : (
        ...
        {semEstoque && (
          <span className="absolute right-3 top-3 rounded-full bg-foreground/85 px-2.5 py-1 text-[11px] font-semibold text-white shadow">
            Esgotado
          </span>
        )}
        {!semEstoque && estoqueBaixo && (
          <span className="absolute right-3 top-3 rounded-full bg-warm px-2.5 py-1 text-[11px] font-semibold text-white shadow">
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-auto flex items-center justify-between pt-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-full bg-primary px-3.5 py-2 text-sm font-semibold text-primary-foreground shadow transition hover:opacity-90 active:scale-95 disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground disabled:shadow-none"
            aria-label={
              semEstoque ? `${p.nome} esgotado` : `Adicionar ${p.nome} ao carrinho`
            }
          >
            <Plus className="h-4 w-4" /> {semEstoque ? "Esgotado" : "Adicionar"}
          </button>
        </div>
      </div>
    </article>
